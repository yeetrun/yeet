...
func TestRunWebAPIHostStorageUsesSelectedHost(t *testing.T) {
	...
	if gotDefaultHost != "yeet-pve1" || gotDefaultReq.Service != "nginx" {
		t.Fatalf("default request = host %q req %#v, want nginx on yeet-pve1", gotDefaultHost, gotDefaultReq)
	}
...
func TestRunWebAPIValidateReturnsCommandPreview(t *testing.T) {
	...
	for _, want := range []string{"yeet run app@yeet-pve1", "--net=svc,lan", "--service-root=tank/apps/app", "--zfs"} {
