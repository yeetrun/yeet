...
func TestServiceShellCommandForVMUsesGuestSSH(t *testing.T) {
	...
	wantOptions := []string{
		...
		"-o", "CheckHostIP=no",
		"-o", "ProxyCommand=yeet --host=yeet-pve1 _vm-ssh-proxy devbox %h %p",
	}
...
func TestVMSSHExecutionPlanForServiceBuildsProxyPlan(t *testing.T) {
	...
	for _, want := range []string{
		...
		"ProxyCommand=yeet --host=yeet-pve1 _vm-ssh-proxy devbox %h %p",
		"yeet-pve1",
	} {
...
func TestVMSSHProxyCommandUsesYeetRPCInsteadOfRootSSH(t *testing.T) {
	...
	for _, want := range []string{"yeet", "--host=yeet-pve1", "_vm-ssh-proxy", "devbox", "%h", "%p"} {
...
func TestServiceShellCommandForVMSvcLANFallsBackToSvcProxyWhenLANUnreachable(t *testing.T) {
	...
	wantProxy := "ProxyCommand=yeet --host=yeet-pve1 _vm-ssh-proxy devbox %h %p"
