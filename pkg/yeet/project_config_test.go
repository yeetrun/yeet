...
func TestPlainPrompterSelectWorkspaceUsesKnownWorkspace(t *testing.T) {
	...
	selection, err := plainPrompter{}.SelectWorkspace("yeet-lab", []string{"/tmp/workspace"}, "/tmp/current")
	if err != nil {
...
func TestPlainPrompterSelectWorkspaceCanRunOnce(t *testing.T) {
	...
	selection, err := plainPrompter{}.SelectWorkspace("yeet-lab", nil, "/tmp/current")
	if err != nil {
		t.Fatalf("SelectWorkspace error: %v", err)
	}
...
func TestLoadProjectConfigFromFile(t *testing.T) {
	...
	if _, ok := loaded.Config.ServiceEntry("sonarr", "yeet-lab"); !ok {
...
func TestSaveRunConfigStoresVMType(t *testing.T) {
	...
	if err := saveRunConfigWithPayloadKind(loc, "yeet-lab", "vm://ubuntu/26.04", serviceTypeVM, []string{"--net=svc", "--vcpus=4"}, "", false); err != nil {
...
func TestProjectConfigSetServiceRootForEntryCanClear(t *testing.T) {
	...
	if cfg.SetServiceRootForEntry("missing", "yeet-lab", "/srv/apps/missing", false) {
...
func TestProjectConfigForWriteLocalConfigWinsOverMalformedClientConfig(t *testing.T) {
	...
	if _, err := seedWorkspaceConfig(projectDir, "yeet-lab"); err != nil {
		t.Fatalf("seedWorkspaceConfig: %v", err)
	}
...
func TestProjectConfigForWriteCanAdoptConfiguredWorkspaceForNewHost(t *testing.T) {
	...
	restore := stubClientConfigState(t, clientConfig{DefaultHost: "yeet-cloud", Workspaces: []string{workspace}})
	defer restore()
	oldPrompt := activePrompter
	oldIsTerminal := isTerminalFn
	var gotPaths []string
	...
	if got := loaded.Config.AllHosts(); !reflect.DeepEqual(got, []string{"yeet-cloud", "yeet-lab"}) {
