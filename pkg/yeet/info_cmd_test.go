...
func TestHandleInfoCommandWithoutServiceRendersHostInfo(t *testing.T) {
	...
	fetchInfoHostInfoFn = func(ctx context.Context, host string) (serverInfo, error) {
		if host != "yeet-pve1" {
			t.Fatalf("Info called with host=%q, want yeet-pve1", host)
		}
		return serverInfo{
			Version:     "v0.9.0",
			GOOS:        "linux",
	...
	fetchInfoServiceInfoFn = func(ctx context.Context, host, service string) (catchrpc.ServiceInfoResponse, error) {
		if host != "yeet-pve1" || service != catchServiceName {
			t.Fatalf("ServiceInfo called with host=%q service=%q, want yeet-pve1/%s", host, service, catchServiceName)
		}
	...
	fetchStatusForHostFn = func(ctx context.Context, host string, flags cli.StatusFlags) ([]statusService, error) {
		if host != "yeet-pve1" {
			t.Fatalf("status called with host=%q, want yeet-pve1", host)
		}
		if flags.Format != "" {
	...
	assertPlainRow(t, out, "Host", "yeet-pve1")
