...
func TestRunVMRsyncCopyUploadBuildsRsyncCommand(t *testing.T) {
	...
	for _, want := range []string{"-avz", "-e", "./local.txt", "./local.pub", "yeet-lab:/etc/motd"} {
	...
	remoteIdx := slices.Index(got.args, "yeet-lab:/etc/motd")
...
func TestRunVMRsyncCopyDownloadBuildsRsyncCommand(t *testing.T) {
	...
	for _, want := range []string{"-avz", "yeet-lab:.ssh/id*", "yeet-lab:~/app.log", "./logs/"} {
		if !slices.Contains(gotArgs, want) {
			t.Fatalf("rsync args = %#v, want %q", gotArgs, want)
		}
	}
	globIdx := slices.Index(gotArgs, "yeet-lab:.ssh/id*")
	logIdx := slices.Index(gotArgs, "yeet-lab:~/app.log")
...
func TestRunRsyncPlanRepairsKnownHostOnce(t *testing.T) {
	...
	plan := sshExecutionPlan{
		Args: []string{"-o", "HostName=192.168.100.12", "yeet-lab"},
...
func TestRunRsyncPlanHintsWhenInitialAttemptFindsMissingGuestRsync(t *testing.T) {
	...
	plan := sshExecutionPlan{Args: []string{"-o", "HostName=192.168.100.12", "yeet-lab"}}
