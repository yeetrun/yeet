...
the failure on `lab-host`, but the bug is broader: any path that recreates a
...
7. Acceptance must include live verification on both `lab-host` and `cloud-host`.
...
seen on `lab-host`: named netns objects were recreated even though the compose
...
Acceptance must include live verification on both `lab-host` and `cloud-host`.
...
On `lab-host`, also verify this against an existing multi-container or
representative compose service where practical.

## Rollout Notes

This fix should repair the exact outage pattern that happened on `lab-host`, but
