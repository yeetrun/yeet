...
chains with bare `iptables` calls. On the inspected live host, `lab-host`, that
...
7. Acceptance must include upgrading `catch` and using `yeet` to deploy to
   `root@lab-host` to confirm the new code works end to end.
...
Acceptance must include end-to-end testing against the real host `lab-host`:

1. Upgrade `catch` on `root@lab-host` with the new code.
...
6. `catch` can be upgraded and a real service can be deployed to `lab-host` using
