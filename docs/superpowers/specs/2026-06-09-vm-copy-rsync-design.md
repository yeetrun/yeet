...
```text
Proxying VM SSH through yeet-lab to 192.168.100.12
```
...
Add live coverage on lab-host with a disposable VM:

```bash
yeet run copy-vm-test@yeet-lab vm://ubuntu/26.04 --net=svc,lan
...
- Live lab-host VM copy succeeds in both upload and download directions.
