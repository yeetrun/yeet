...
The live `yeet-lab` migration showed the gap. The requested data dir and
...
The live `yeet-lab` migration reached `/flash/yeet/services`, but left
...
host. `yeet info --host=yeet-lab` should do the same for an explicit host.
...
```text
Host storage plan for yeet-lab
...
Focused live validation on `yeet-lab` should verify:
...
- `yeet --host yeet-lab status --progress=plain` shows all previously running
  services running
- `yeet info --host=yeet-lab` reports `/flash/yeet/data`,
