...
- Keep VMs experimental and scoped while preserving the existing service
  workflow.
- Boot a yeet-owned Ubuntu VM on `yeet-pve1` into a usable serial console in
  under five seconds.
...
- `yeet-pve1` is Debian 13 / Proxmox with 12 logical CPUs, about 31 GiB RAM,
  `/dev/kvm`, `qemu-img`, and ZFS pools including `flash`.
- `yeet-hetz` is Ubuntu 24.04 with 2 logical CPUs and about 1.9 GiB RAM, but it
...
console or a guest-ready marker in under five seconds on `yeet-pve1`. SSH
...
If `yeet-hetz` exposed KVM with its current size:
...
```text
VMs require Linux KVM; /dev/kvm is missing on host "yeet-hetz"
...
- Sizing:
  - pve1-like profile defaults to 4 vCPU, 4 GiB RAM
...
CATCH_HOST=yeet-pve1 go run ./cmd/yeet ssh devbox
CATCH_HOST=yeet-pve1 go run ./cmd/yeet info devbox
```

Acceptance:

- On `yeet-pve1`, the VM reaches a usable serial login or guest-ready marker in
...
- `yeet-hetz` reports missing KVM clearly and does not create a partial VM.
