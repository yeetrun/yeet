...
Fresh disposable VMs on `pve1` showed that the kernel path is already similar
...
Publish a new NixOS image version and test on `pve1` with disposable VMs.
