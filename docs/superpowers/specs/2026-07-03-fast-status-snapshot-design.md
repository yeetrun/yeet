...
across `yeet-pve1` and `yeet-hetz` in roughly 1.1 to 1.8 seconds. Timed host
runs showed `yeet-pve1` dominates at roughly 1.15 to 1.62 seconds, while
`yeet-hetz` finishes in roughly 0.36 to 0.42 seconds.

The client already queries configured hosts concurrently for table status, so
the main delay is catch-side collection on each host. On `yeet-pve1`, catch
...
/usr/bin/time -p yeet --host yeet-pve1 status --format=json >/tmp/yeet-pve1-status.json
```
...
with the biggest improvement on `yeet-pve1`.
