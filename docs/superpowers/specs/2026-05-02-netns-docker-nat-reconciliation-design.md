...
After the `lab-host` reboot and catch update, `hoarder` was reported as down even
...
8. Existing link-level reconciliation must remain in place for stale netns/veth
   problems.
9. Acceptance must include live verification on `lab-host` against affected
   services.
...
After deploying updated catch to `lab-host`:
...
2. `go test ./...` passes.
3. The updated catch binary deployed to `lab-host` restores flushed hoarder NAT
