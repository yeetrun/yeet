...
- Publish from GitHub Actions and test live on `pve1` with disposable VMs.
...
After publishing a test NixOS image, validate on `pve1` with disposable VMs.

Raw disk:

```bash
yeet run nixos-smoke@yeet-pve1 vm://nixos/26.05
yeet ssh nixos-smoke@yeet-pve1
...
```bash
yeet run nixos-zfs@yeet-pve1 vm://nixos/26.05 \
  --service-root=flash/yeet/vms/nixos-zfs \
  --zfs \
  --disk=128g \
  --net=lan
yeet ssh nixos-zfs@yeet-pve1
