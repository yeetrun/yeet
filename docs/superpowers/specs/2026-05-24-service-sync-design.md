...
- If no matching entry exists, return a clear error:
  ```text
  no yeet.toml entry for sonarr@yeet-pve1; run with --config from the project directory or add the service first
  ```
...
```text
service "sonarr" not found on yeet-pve1
```

```text
no yeet.toml entry for sonarr@yeet-pve1
```
...
```text
Updated sonarr@yeet-pve1
Skipped radarr@yeet-pve1: service not found on catch
1 updated, 1 skipped
```
