...
- Avoid a general migration framework. The only existing VM host is
  `root@pve1`, so existing VM cleanup can be handled with one-off surgical
...
After the code lands, update `root@pve1` VMs directly. The acceptable approach
