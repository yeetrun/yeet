...
```bash
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet copy ~/.ssh/id* <disposable-vm>:.ssh/
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet copy <disposable-vm>:".ssh/id*" ./.tmp-copy-smoke/
```

Use a disposable VM on `yeet-pve1` for guest filesystem validation and clean it
