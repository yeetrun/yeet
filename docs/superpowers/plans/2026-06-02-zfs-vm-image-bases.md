...
## Task 7: Live ZFS VM End-to-End Validation on `yeet-pve1`
...
- [ ] **Step 1: Deploy the updated catch binary to pve1**

Run:

```bash
CATCH_HOST=yeet-pve1 go run ./cmd/yeet init yeet-pve1
```
...
```bash
CATCH_HOST=yeet-pve1 go run ./cmd/yeet run vmzfs-a vm://ubuntu/26.04 --service-root=flash/yeet/vms/vmzfs-a --zfs --disk=128g --net=lan
```
...
```bash
ssh yeet-pve1 zfs list -H -o name,origin,volsize flash/yeet/vm-images/ubuntu-26.04-amd64-v1/root flash/yeet/vms/vmzfs-a/vm
```
...
```bash
CATCH_HOST=yeet-pve1 go run ./cmd/yeet run vmzfs-b vm://ubuntu/26.04 --service-root=flash/yeet/vms/vmzfs-b --zfs --disk=128g --net=lan
```
...
```bash
CATCH_HOST=yeet-pve1 go run ./cmd/yeet rm vmzfs-a --clean-data -y
CATCH_HOST=yeet-pve1 go run ./cmd/yeet rm vmzfs-b --clean-data -y
```
...
```bash
ssh yeet-pve1 zfs list -H -o name flash/yeet/vm-images/ubuntu-26.04-amd64-v1/root
```
