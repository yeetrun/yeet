...
Add executor and cleanup functions in the same file. The LAN path should use direct tap attachment when the parent is a bridge. Add a second backend for non-bridge parents only after a local lab-host-compatible proof command shows Firecracker can attach the chosen host device.
...
func TestRenderInfoPlainIncludesVMSection(t *testing.T) {
	...
	var out bytes.Buffer
	err := renderInfoPlain(&out, "devbox", "yeet-lab", nil, serverInfo{}, clientInfo{}, server)
	if err != nil {
...
### Task 12: Live Validation on `yeet-lab`

**Files:**
- No source files unless validation exposes a defect.

- [ ] **Step 1: Verify lab-host host prerequisites**

Run:

```bash
ssh root@lab-host 'uname -a; test -e /dev/kvm && echo kvm=yes; command -v firecracker || true; command -v qemu-img; command -v zfs || true; ip route show default'
```

Expected: output includes `kvm=yes`, `qemu-img`, and a default route. Firecracker may be absent because yeet caches the bundle.

- [ ] **Step 2: Run remote unit workflow against lab-host**
...
```bash
ssh root@lab-host 'systemd-analyze timestamp now; systemctl restart yeet-vm-devbox.service; timeout 5 journalctl -u yeet-vm-devbox.service -f -o cat | sed -n "/guest-ready/q"'
```

Expected: guest-ready marker appears within five seconds on lab-host. If serial login appears before the marker, record the serial login time as passing the boot target.
...
CATCH_HOST=yeet-lab go run ./cmd/yeet ssh devbox -- uname -a
```

Expected: console reaches the Ubuntu serial login or shell, and SSH prints the guest kernel line without manual key setup.

- [ ] **Step 5: Verify `yeet-cloud` no-KVM error**

Run:

```bash
CATCH_HOST=yeet-cloud go run ./cmd/yeet run cloudvm vm://ubuntu/26.04 --net=svc
```
