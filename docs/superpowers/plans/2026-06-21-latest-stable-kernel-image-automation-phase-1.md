...
```bash
cd /Users/shayne/code/yeet
CATCH_HOST=yeet-pve1 go run ./cmd/yeet vm images update vm://ubuntu/26.04
CATCH_HOST=yeet-pve1 go run ./cmd/yeet vm images update vm://nixos/26.05
CATCH_HOST=yeet-pve1 go run ./cmd/yeet run ubuntu-kernel-smoke vm://ubuntu/26.04 --image-policy=update
CATCH_HOST=yeet-pve1 go run ./cmd/yeet run nixos-kernel-smoke vm://nixos/26.05 --image-policy=update
CATCH_HOST=yeet-pve1 go run ./cmd/yeet ssh ubuntu-kernel-smoke -- 'uname -r && whoami && systemctl --failed --no-pager && systemctl show -p SystemState -p Tainted -p NFailedUnits && nft --version && iptables --version && test -c /dev/net/tun'
CATCH_HOST=yeet-pve1 go run ./cmd/yeet ssh nixos-kernel-smoke -- 'uname -r && whoami && systemctl --failed --no-pager && systemctl show -p SystemState -p Tainted -p NFailedUnits && nixos-version && nix --version && nft --version && iptables --version && test -c /dev/net/tun'
```
...
```bash
cd /Users/shayne/code/yeet
CATCH_HOST=yeet-pve1 go run ./cmd/yeet rm ubuntu-kernel-smoke --clean-data
CATCH_HOST=yeet-pve1 go run ./cmd/yeet rm nixos-kernel-smoke --clean-data
```
