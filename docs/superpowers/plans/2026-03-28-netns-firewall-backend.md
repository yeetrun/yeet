...
### Task 7: End-To-End Validation On `lab-host`

**Files:**
- Verify live host state, no repo file edits expected

- [ ] **Step 1: Upgrade `catch` on `lab-host` from the current checkout**
...
```bash
CATCH_HOST=yeet-lab go run ./cmd/yeet run netfw-check nginx:latest --net=svc
```
...
```bash
svc_ip=$(CATCH_HOST=yeet-lab go run ./cmd/yeet ip netfw-check | tr -d '\r')
ssh root@lab-host "ip netns list; nft list table ip yeet || true; iptables -S YEET_FORWARD || true; iptables -S YEET_POSTROUTING || true"
```
...
```bash
svc_ip=$(CATCH_HOST=yeet-lab go run ./cmd/yeet ip netfw-check | tr -d '\r')
ssh root@lab-host "curl -fsSI http://${svc_ip}:80 | head -n 1"
```
...
```bash
CATCH_HOST=yeet-lab go run ./cmd/yeet run netfw-check nginx:latest --net=svc
ssh root@lab-host "nft list table ip yeet || true; iptables -S YEET_FORWARD || true"
```
...
```bash
svc_ip=$(CATCH_HOST=yeet-lab go run ./cmd/yeet ip netfw-check | tr -d '\r')
ssh root@lab-host "rm -f /tmp/iperf-netfw.log; nohup ip netns exec yeet-netfw-check-ns iperf3 -s -1 -B ${svc_ip} -p 25202 > /tmp/iperf-netfw.log 2>&1 < /dev/null &"
ssh root@lab-host "iptables -t nat -C PREROUTING -i vmbr0 -p tcp --dport 25212 -j DNAT --to-destination ${svc_ip}:25202 2>/dev/null || iptables -t nat -I PREROUTING 1 -i vmbr0 -p tcp --dport 25212 -j DNAT --to-destination ${svc_ip}:25202"
ssh shayne@macstudio.local 'nix shell nixpkgs#iperf3 -c iperf3 -c 10.0.4.2 -p 25212 -P 4 -t 10 -O 2 -f g'
ssh root@lab-host "while iptables -t nat -C PREROUTING -i vmbr0 -p tcp --dport 25212 -j DNAT --to-destination ${svc_ip}:25202 2>/dev/null; do iptables -t nat -D PREROUTING -i vmbr0 -p tcp --dport 25212 -j DNAT --to-destination ${svc_ip}:25202; done"
```
...
```bash
CATCH_HOST=yeet-lab go run ./cmd/yeet rm netfw-check --yes --clean-config
```
