...
- [ ] **Step 4: Deploy and verify on `lab-host`**
...
- `CATCH_HOST=yeet-lab go run ./cmd/yeet status` returns promptly after restart
...
- [ ] **Step 5: Deploy and verify on `cloud-host`**
...
Verify:
- `CATCH_HOST=yeet-cloud go run ./cmd/yeet status` returns promptly after restart
