...
- [ ] **Step 4: Run live read-only and VM smoke checks on pve1**

Run:

```bash
CATCH_HOST=yeet-pve1 go run ./cmd/yeet status
CATCH_HOST=yeet-pve1 go run ./cmd/yeet info devbox
CATCH_HOST=yeet-pve1 go run ./cmd/yeet ssh -o BatchMode=yes -o ConnectTimeout=10 devbox -- hostname
```
