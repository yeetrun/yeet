...
**Architecture:** Keep this pass image-first. Update the root and image-repo build scripts to purge/mask only high-confidence residual Ubuntu services, harden workflow verification for the cleanup set, publish and live-test the image on `pve1`, then bump the catch default/docs after the image exists.
