...
## Task 4: Live-Test v5 on lab-host Before Adopting It
...
mise exec -- /tmp/yeet-vmfast-v5 init root@lab-host
```
...
cmd = [
    "/tmp/yeet-vmfast-v5", "run", f"{svc}@yeet-lab", "vm://ubuntu/26.04",
...
/tmp/yeet-vmfast-v5 ssh "$svc@yeet-lab" -- true
/tmp/yeet-vmfast-v5 ssh "$svc@yeet-lab" -- systemd-analyze | tee ".tmp/${svc}-systemd-analyze.txt"
/tmp/yeet-vmfast-v5 ssh "$svc@yeet-lab" -- 'systemd-analyze blame | head -n 30' | tee ".tmp/${svc}-systemd-blame.txt"
/tmp/yeet-vmfast-v5 ssh "$svc@yeet-lab" -- 'dmesg | tail -n 80' | tee ".tmp/${svc}-dmesg-tail.txt"
/tmp/yeet-vmfast-v5 ssh "$svc@yeet-lab" -- systemctl status yeet-sshd.service yeet-guest-ready.service --no-pager | tee ".tmp/${svc}-guest-services.txt"
```
...
```bash
/tmp/yeet-vmfast-v5 ssh "$svc@yeet-lab" -- 'for p in netplan.io networkd-dispatcher chrony sysstat; do dpkg-query -W "$p" >/dev/null 2>&1 && echo "installed:$p" || echo "absent:$p"; done' | tee ".tmp/${svc}-package-trim.txt"
/tmp/yeet-vmfast-v5 ssh "$svc@yeet-lab" -- 'for u in netplan-configure.service networkd-dispatcher.service sysstat.service e2scrub_reap.service ldconfig.service keyboard-setup.service chrony.service; do printf "%s " "$u"; systemctl is-enabled "$u" 2>/dev/null || true; done' | tee ".tmp/${svc}-unit-trim.txt"
```
...
```bash
/tmp/yeet-vmfast-v5 ssh "$svc@yeet-lab" -- sudo reboot || true
sleep 3
ssh root@lab-host "systemctl is-active yeet-vm-$svc.service" | tee ".tmp/${svc}-reboot-unit-active.txt"
start="$(date +%s)"
ok=0
for i in $(seq 1 45); do
  if /tmp/yeet-vmfast-v5 ssh "$svc@yeet-lab" -- true >/dev/null 2>&1; then
    ok=1
    break
  fi
  sleep 1
...
test "$ok" = 1
ssh root@lab-host "systemctl status yeet-vm-$svc.service --no-pager" | tee ".tmp/${svc}-reboot-host-unit.txt"
...
set timeout 120
spawn /tmp/yeet-vmfast-v5 rm ${svc}@yeet-lab --clean-data
...
PY
ssh root@lab-host "zfs list -H -o name -r flash/yeet/vms/$svc 2>/dev/null || true" | tee ".tmp/${svc}-zfs-leftovers.txt"
ssh root@lab-host "systemctl list-units --all 'yeet-vm-$svc.service' --no-legend || true" | tee ".tmp/${svc}-unit-leftovers.txt"
```
...
- [ ] **Step 2: Confirm lab-host cleanup**

Run:

```bash
ssh root@lab-host "zfs list -H -o name -r flash/yeet/vms 2>/dev/null | grep -E 'bootv5' || true; systemctl list-units --all 'yeet-vm-bootv5*.service' --no-legend || true"
```
