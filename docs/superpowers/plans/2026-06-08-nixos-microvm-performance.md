...
**Architecture:** Keep the change image-side in `/Users/shayne/code/yeet-vm-images`. Add an explicit NixOS microVM profile that disables module-loading work through NixOS declarations, add CI verification for the evaluated boot graph, publish a new NixOS image, and compare first boot plus reboot live on `lab-host`.

**Tech Stack:** Nix/NixOS 26.05 modules, Bash verification scripts, GitHub Actions, Firecracker, Linux 7.0 yeet kernel, ext4, systemd, yeet/catch live testing on `lab-host`.
...
The previous live comparison on `lab-host` produced:
...
nix eval --extra-experimental-features 'nix-command flakes' --json .#nixosConfigurations.yeet-nixos-26_05.config.systemd.services.systemd-modules-load.wantedBy
# ["multi-user.target"]
```

The new image should remove avoidable module-loading work while preserving normal NixOS management.
...
Expected: FAIL before implementation. The failure should mention `boot.kernelModules`, `systemd-modules-load`, or one of the `modprobe@...` services.
...
- [ ] **Step 2: Extend the `boot` declaration**

Replace the existing `boot` block:
...
```bash
nix eval --extra-experimental-features 'nix-command flakes' --json .#nixosConfigurations.yeet-nixos-26_05.config.boot.kernelModules
nix eval --extra-experimental-features 'nix-command flakes' --json .#nixosConfigurations.yeet-nixos-26_05.config.boot.extraModulePackages
nix eval --extra-experimental-features 'nix-command flakes' --json .#nixosConfigurations.yeet-nixos-26_05.config.systemd.services.systemd-modules-load.wantedBy
```
...
### Task 6: Update lab-host Cache And Run First-Boot/Reboot Comparison

**Files:**
- No local file edits

- [ ] **Step 1: Update the NixOS image cache on lab-host**

From `/Users/shayne/code/yeet`, run:

```bash
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet vm images update vm://nixos/26.05
```
...
```bash
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet rm cmp-ubuntu-perf-0608 --clean-data --clean-config --yes >/dev/null 2>&1 || true
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet rm cmp-nixos-perf-0608 --clean-data --clean-config --yes >/dev/null 2>&1 || true
```
...
```bash
/usr/bin/time -p env CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet run cmp-ubuntu-perf-0608 vm://ubuntu/26.04 --net=svc
/usr/bin/time -p env CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet run cmp-nixos-perf-0608 vm://nixos/26.05 --net=svc
```
...
```bash
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet ssh cmp-ubuntu-perf-0608 -- sudo dmesg > /tmp/yeet-ubuntu-perf-first-dmesg.txt
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet ssh cmp-nixos-perf-0608 -- sudo dmesg > /tmp/yeet-nixos-perf-first-dmesg.txt
```
...
```bash
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet ssh cmp-ubuntu-perf-0608 -- cat /proc/sys/kernel/random/boot_id > /tmp/yeet-ubuntu-perf-first-boot-id.txt
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet ssh cmp-nixos-perf-0608 -- cat /proc/sys/kernel/random/boot_id > /tmp/yeet-nixos-perf-first-boot-id.txt
```
...
```bash
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet ssh cmp-ubuntu-perf-0608 -- sudo reboot >/tmp/yeet-ubuntu-perf-reboot.out 2>/tmp/yeet-ubuntu-perf-reboot.err || true
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet ssh cmp-nixos-perf-0608 -- sudo reboot >/tmp/yeet-nixos-perf-reboot.out 2>/tmp/yeet-nixos-perf-reboot.err || true
```
...
wait_changed() {
  ...
  for i in $(seq 1 90); do
    out=$(CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet ssh "$svc" -- cat /proc/sys/kernel/random/boot_id 2>/dev/null || true)
...
```bash
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet ssh cmp-ubuntu-perf-0608 -- sudo dmesg > /tmp/yeet-ubuntu-perf-second-dmesg.txt
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet ssh cmp-nixos-perf-0608 -- sudo dmesg > /tmp/yeet-nixos-perf-second-dmesg.txt
```
...
```bash
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet ssh cmp-nixos-perf-0608 -- /run/current-system/sw/bin/systemctl is-system-running
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet ssh cmp-nixos-perf-0608 -- /run/current-system/sw/bin/systemctl --failed --no-pager
```
...
```bash
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet ssh cmp-nixos-perf-0608 -- /run/current-system/sw/bin/nix show-config > /tmp/yeet-nixos-perf-nix-show-config.txt
grep -E '^experimental-features = .*nix-command.*flakes' /tmp/yeet-nixos-perf-nix-show-config.txt
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet ssh cmp-nixos-perf-0608 -- /run/current-system/sw/bin/nix flake --help > /tmp/yeet-nixos-perf-nix-flake-help.txt
```
...
```bash
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet ssh cmp-nixos-perf-0608 -- sudo nixos-rebuild switch
```
...
```bash
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet rm cmp-ubuntu-perf-0608 --clean-data --clean-config --yes
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet rm cmp-nixos-perf-0608 --clean-data --clean-config --yes
ssh root@lab-host 'systemctl list-unit-files "yeet-cmp-ubuntu-perf-0608.service" "yeet-cmp-nixos-perf-0608.service" --no-legend --no-pager 2>/dev/null || true; ls -ld /root/data/services/cmp-ubuntu-perf-0608 /root/data/services/cmp-nixos-perf-0608 2>/dev/null || true'
```
...
```bash
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet vm images
```
