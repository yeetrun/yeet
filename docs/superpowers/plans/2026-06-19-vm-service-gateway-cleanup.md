...
**Goal:** Make yeet VMs use `192.168.100.1` as the service-network gateway and verify existing pve1 VMs are surgically refreshed without adding migration code.
...
**Tech Stack:** Go, Linux network namespaces, Firecracker VM boot args, injected guest metadata, GitButler, pve1 live host verification.
...
## Task 4: Deploy Catch and Refresh Existing pve1 VMs

**Files:**
- No repo file changes expected.

- [ ] **Step 1: Install the new catch build on pve1**
...
```bash
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet status
ssh root@pve1 'find /root/.local/share/yeet -path "*metadata/network.yaml" -o -path "*etc/yeet-vm/systemd-network/10-yeet-eth0.network" 2>/dev/null | sort'
```

Expected: identify the VMs that still reference `192.168.100.254`.

- [ ] **Step 3: Surgically update pve1 VM guest network config**
...
```bash
ssh root@pve1 'grep -R "192.168.100.254" -n /root/.local/share/yeet /var/lib/yeet 2>/dev/null || true'
```
...
```bash
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet vm stop nixbox
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet vm start nixbox
```
...
- [ ] **Step 3: Check pve1 sidecar and namespace state**

Run:

```bash
ssh root@pve1 'ip netns exec yeet-ns ip route show; ip route show 192.168.100.0/24; systemctl --no-pager --failed "yeet-*"; true'
```
