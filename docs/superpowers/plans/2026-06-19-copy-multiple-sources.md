...
func TestRunVMRsyncCopyUploadIncludesMultipleSources(t *testing.T) {
	...
	assertSubsequence(t, gotArgs, []string{"-avz", "-e", "./id_ed25519", "./id_ed25519.pub", "yeet-lab:.ssh/"})
}
...
func TestRunVMRsyncCopyDownloadPassesRemoteGlob(t *testing.T) {
	...
	assertSubsequence(t, gotArgs, []string{"-avz", "-e", "yeet-lab:.ssh/id*", "./keys/"})
}
```
...
- [ ] **Step 1: Create a disposable VM on lab-host**

Create a uniquely named disposable VM so cleanup is unambiguous:

```bash
VM_NAME=codex-copy-smoke-$(date +%Y%m%d%H%M%S)
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet run "$VM_NAME" vm://ubuntu/26.04 --net=svc
```

Verify SSH readiness:

```bash
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet ssh "$VM_NAME" -- true
```
...
printf 'alpha\n' > "$tmpdir/a.txt"
printf 'beta\n' > "$tmpdir/b.txt"
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet copy "$tmpdir/a.txt" "$tmpdir/b.txt" "${VM_NAME}:/tmp/yeet-copy-smoke/"
```

Verify in the VM:

```bash
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet ssh "$VM_NAME" -- 'cat /tmp/yeet-copy-smoke/a.txt /tmp/yeet-copy-smoke/b.txt'
```
...
```bash
outdir=$(mktemp -d)
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet copy "${VM_NAME}:/tmp/yeet-copy-smoke/*.txt" "$outdir/"
ls -1 "$outdir"
...
printf '#!/bin/sh\nsleep infinity\n' > "$script"
chmod +x "$script"
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet run "$REGULAR_NAME" "$script"
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet copy "${REGULAR_NAME}:logs/*.txt" "$outdir/"
```
...
```bash
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet remove "$VM_NAME" --yes --clean-data --clean-config
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet remove "$REGULAR_NAME" --yes --clean-data --clean-config
```
