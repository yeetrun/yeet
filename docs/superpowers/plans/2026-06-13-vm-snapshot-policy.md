...
- [ ] **Step 3: Live test on `yeet-pve1` with a disposable ZFS-backed VM**

Use a disposable service name that does not collide with existing services, for example:

```bash
export CATCH_HOST=yeet-pve1
...
ssh root@pve1 "zfs list -H -t snapshot -o name,com.yeetrun:service,com.yeetrun:event,com.yeetrun:comment | grep codex-vm-snap-test"
```
