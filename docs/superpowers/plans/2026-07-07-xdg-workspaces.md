...
func TestProjectConfigForWritePromptsToRegisterLocalConfig(t *testing.T) {
	cwd, _ := os.Getwd()
	tmp := t.TempDir()
	if _, err := seedWorkspaceConfig(tmp, "yeet-pve1"); err != nil {
		t.Fatalf("seedWorkspaceConfig: %v", err)
	}
...
func TestFinishInitWorkspaceSetupExplicitWorkspaceCreatesSeedAndSavesConfig(t *testing.T) {
	...
	ui := newInitUI(io.Discard, false, true, "yeet-pve1", "root@example.com", catchServiceName)
	...
	if !strings.Contains(string(raw), `hosts = ["yeet-pve1"]`) {
	...
	if !strings.Contains(string(configRaw), `default_host = "yeet-pve1"`) || !strings.Contains(string(configRaw), workspace) {
...
mkdir -p "$workspace"
XDG_CONFIG_HOME="$tmp/xdg" mise exec -- go run ./cmd/yeet config --host Yeet-PVE1 --workspace "$workspace"
