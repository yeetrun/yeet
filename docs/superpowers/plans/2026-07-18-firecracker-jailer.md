...
**Goal:** Launch new Yeet Firecracker VMMs through the matching Firecracker jailer as the unprivileged `yeet-vm` host identity, support explicit one-VM migration, and migrate the live VMs on `yeet-pve1` safely.
...
### Task 10: Isolated Live Rollout on `yeet-pve1`
...
For every Firecracker version referenced by a VM on `root@pve1`, fetch the
...
Install the isolated Catch revision on `yeet-pve1`. Create one uniquely named
