...
## Task 7: Deploy To LabHost And Verify NAT Repair

**Files:**
- No repo file changes expected.

- [ ] **Step 1: Install updated catch on lab-host**

Run:

```bash
go run ./cmd/yeet --progress=plain init root@yeet-lab
```
...
```bash
go run ./cmd/yeet --host yeet-lab status | rg '^(hoarder|plex|prowlarr|duplicati)'
```
...
```bash
ssh -o HostKeyAlias=lab-host -o UpdateHostKeys=no root@yeet-lab "docker ps --filter name=catch-hoarder --format '{{.Names}} {{.ID}} {{.Status}}'; echo __NAT__; ip netns exec yeet-hoarder-ns iptables -t nat -S YEET_PREROUTING; echo __OUTPUT__; ip netns exec yeet-hoarder-ns iptables -t nat -S YEET_OUTPUT"
```
...
```bash
ssh -o HostKeyAlias=lab-host -o UpdateHostKeys=no root@yeet-lab "systemctl restart catch.service && sleep 3 && systemctl is-active catch.service"
```
...
```bash
ssh -o HostKeyAlias=lab-host -o UpdateHostKeys=no root@yeet-lab "since=\$(date --iso-8601=seconds); docker restart catch-hoarder-web-1 >/tmp/hoarder-restart.out; sleep 2; cat /tmp/hoarder-restart.out; journalctl -u catch -u docker --since \"\$since\" --no-pager | grep -F 'NetworkDriver.RevokeExternalConnectivity: Not implemented' || true"
```
