...
- [ ] **Step 6: Install local yeet and catch on lab-host**
...
- [ ] **Step 7: Validate remote catalog behavior on lab-host**

Run:

```bash
cd /Users/shayne/code/yeet
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet vm images catalog --format=json-pretty
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet vm images update vm://ubuntu/26.04 --format=json-pretty
```
...
CATCH_HOST=yeet-lab yeet run codex-agent-ubuntu vm://ubuntu/26.04 --service-root=flash/yeet/vms/codex-agent-ubuntu --zfs --disk=16g --net=lan --image-policy=update
CATCH_HOST=yeet-lab yeet ip codex-agent-ubuntu
CATCH_HOST=yeet-lab yeet ssh codex-agent-ubuntu -- hostname
CATCH_HOST=yeet-lab yeet rm codex-agent-ubuntu --yes --clean-data --clean-config
```
