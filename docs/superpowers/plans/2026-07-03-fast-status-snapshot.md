...
- [ ] **Step 7: Time the lab-host host path**

Run:

```bash
cd /Users/shayne/yeet-services
for i in 1 2 3; do /usr/bin/time -p yeet --host yeet-lab status --format=json >/tmp/yeet-status-lab-host-$i.json; done
```

Expected: all three runs exit 0 and the `real` times are materially below the
measured 1.15 to 1.62 second baseline for `yeet-lab`.
