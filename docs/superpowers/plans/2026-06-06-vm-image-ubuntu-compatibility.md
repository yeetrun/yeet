...
**Tech Stack:** Bash image builders, GitHub Actions, Go tests, Firecracker/Ubuntu rootfs validation, yeet/catch live testing on `lab-host`, website changelog release flow.
...
### Task 7: Live Validate v12 On lab-host
...
- [ ] **Step 1: Build local yeet and upgrade catch on lab-host**

Run:

```bash
go build -o /tmp/yeet-v12-compat ./cmd/yeet
/tmp/yeet-v12-compat --host=yeet-lab init root@lab-host
```
...
```bash
/tmp/yeet-v12-compat --host=yeet-lab --progress=plain run vmcompat-v12 vm://ubuntu/26.04 \
...
```bash
/tmp/yeet-v12-compat --host=yeet-lab ssh vmcompat-v12 -- '
set -eu
uname -r
...
```bash
/tmp/yeet-v12-compat --host=yeet-lab --progress=plain run vmcompat-ts-v12 vm://ubuntu/26.04 \
  --service-root=flash/yeet/vms/vmcompat-ts-v12 \
  --zfs \
  --disk=16g \
  --net=lan \
  --image-policy=cached
/tmp/yeet-v12-compat --host=yeet-lab ssh vmcompat-ts-v12 -- '
set -eu
...
/tmp/yeet-v12-compat --host=yeet-lab ssh vmcompat-ts-v12 -- "sudo tailscale up --authkey='$TAILSCALE_AUTHKEY' --hostname=yeet-vmcompat-ts-v12"
/tmp/yeet-v12-compat --host=yeet-lab ssh vmcompat-ts-v12 -- "tailscale status --peers=false"
/tmp/yeet-v12-compat --host=yeet-lab ssh vmcompat-ts-v12 -- "sudo tailscale logout"
```
...
```bash
/tmp/yeet-v12-compat --host=yeet-lab rm vmcompat-v12 --clean-data --yes
/tmp/yeet-v12-compat --host=yeet-lab rm vmcompat-ts-v12 --clean-data --yes
ssh root@lab-host 'zfs list -H -o name | grep -E "flash/yeet/vms/(vmcompat-v12|vmcompat-ts-v12)" && exit 1 || true'
```
...
- Spec coverage: Tasks cover guardrails, root builder fix, validation,
  image-repo mirror, public image build, yeet default adoption, live lab-host
