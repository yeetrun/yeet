...
- [ ] **Step 6: Live validation on `root@pve1` only after user authorization**
...
Expected: catch installs successfully on `root@pve1`.

Then run:

```bash
ssh root@pve1 'bash -lc '"'"'
set -e
echo RESOLV_BEFORE
cat /etc/resolv.conf
...
- Existing `root@pve1` services: Task 4 adds runtime reconciliation and Task 5 validates the live host after explicit authorization.
