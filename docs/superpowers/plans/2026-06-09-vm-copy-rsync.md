...
**Tech Stack:** Go, `pkg/yeet` client orchestration, `pkg/cli` command metadata, catch RPC `ServiceInfo`, local `ssh` and `rsync`, website MDX docs, yeet-vm-images Bash/Nix image builders, pve1 live VM validation.
...
func TestVMSSHExecutionPlanForServiceBuildsProxyPlan(t *testing.T) {
	...
	for _, want := range []string{
		...
		"ProxyCommand=ssh -o StrictHostKeyChecking=accept-new -o UserKnownHostsFile=" + filepath.Join(home, ".yeet", "known_hosts") + " -o HostKeyAlias=yeet-proxy@yeet-pve1 -o CheckHostIP=no -W %h:%p root@yeet-pve1",
		"root@yeet-pve1",
	} {
...
func TestRunVMRsyncCopyUploadBuildsRsyncCommand(t *testing.T) {
	...
	for _, want := range []string{"-avz", "-e", "./local.txt", "root@yeet-pve1:/etc/motd"} {
...
func TestRunVMRsyncCopyDownloadBuildsRsyncCommand(t *testing.T) {
	...
	for _, want := range []string{"-avz", "root@yeet-pve1:~/app.log", "./logs/"} {
...
func TestRunRsyncPlanRepairsKnownHostOnce(t *testing.T) {
	...
	plan := sshExecutionPlan{
		Args: []string{"-o", "HostName=192.168.100.12", "root@yeet-pve1"},
...
- [ ] **Step 4: Update catch on pve1**

Run:

```bash
/tmp/yeet-vm-copy --host=yeet-pve1 init root@pve1
```
...
/tmp/yeet-vm-copy --host=yeet-pve1 run "$svc" vm://ubuntu/26.04 --net=svc,lan
/tmp/yeet-vm-copy --host=yeet-pve1 copy "$tmp/source.txt" "$svc:~/source.txt"
/tmp/yeet-vm-copy --host=yeet-pve1 ssh "$svc" -- sh -lc 'test "$(cat ~/source.txt)" = "hello from yeet copy" && command -v rsync'
/tmp/yeet-vm-copy --host=yeet-pve1 copy "$svc:~/source.txt" "$tmp/downloaded.txt"
cmp "$tmp/source.txt" "$tmp/downloaded.txt"
/tmp/yeet-vm-copy --host=yeet-pve1 rm "$svc" --clean-data --yes
rm -rf "$tmp"
```
...
/tmp/yeet-vm-copy --host=yeet-pve1 run "$svc" vm://nixos/26.05 --net=svc
/tmp/yeet-vm-copy --host=yeet-pve1 copy "$tmp/source.txt" "$svc:~/source.txt"
/tmp/yeet-vm-copy --host=yeet-pve1 ssh "$svc" -- sh -lc 'test "$(cat ~/source.txt)" = "hello from nixos copy" && command -v rsync'
/tmp/yeet-vm-copy --host=yeet-pve1 copy "$svc:~/source.txt" "$tmp/downloaded.txt"
cmp "$tmp/source.txt" "$tmp/downloaded.txt"
/tmp/yeet-vm-copy --host=yeet-pve1 rm "$svc" --clean-data --yes
rm -rf "$tmp"
...
SH
chmod +x "$tmp/app.sh"
/tmp/yeet-vm-copy --host=yeet-pve1 run "$svc" "$tmp/app.sh"
printf 'regular service data\n' >"$tmp/config.txt"
/tmp/yeet-vm-copy --host=yeet-pve1 copy "$tmp/config.txt" "$svc:config.txt"
/tmp/yeet-vm-copy --host=yeet-pve1 copy "$svc:config.txt" "$tmp/downloaded-service.txt"
...
- Spec coverage: VM endpoint guest copy, regular service preservation, `--force-proxy`, rsync dependency, direct/proxy transport, missing-rsync errors, image package availability, docs, tests, live pve1 verification, and patch release are each covered by tasks.
