...
- `root@lab-host` is the machine SSH host for catch alias `yeet-lab`; `root@cloud-host` is the machine SSH host for catch alias `yeet-cloud`.
...
```bash
rg -n 'yeet-lab|yeet-cloud|root@lab-host|root@cloud-host|shayne' website/docs/concepts/networking.mdx website/docs/payloads/vms.mdx
...
```bash
ssh root@lab-host "ss -lunpt 'sport = :53' || true; ip -br addr show yeet0 || true"
ssh root@cloud-host "ss -lunpt 'sport = :53' || true; ip -br addr show yeet0 || true"
```
...
- [ ] **Step 3: Install updated local-source catch on yeet-lab and yeet-cloud**

Use the repo's local command path:

```bash
mise exec -- go run ./cmd/yeet --host yeet-lab init root@lab-host
mise exec -- go run ./cmd/yeet --host yeet-cloud init root@cloud-host
```
...
```bash
ssh root@lab-host "systemctl is-active yeet-dns.service && ss -lunp 'sport = :53' | grep 192.168.100.1"
ssh root@cloud-host "systemctl is-active yeet-dns.service && ss -lunp 'sport = :53' | grep 192.168.100.1"
```

Expected: `active` and a UDP listener on `192.168.100.1:53` for both hosts.

- [ ] **Step 5: Smoke service-netns DNS on yeet-lab**

Create or reuse two throwaway `svc` services on `yeet-lab`:

```bash
smoke_payload="$(mktemp /tmp/yeet-dns-smoke.XXXXXX.py)"
printf 'import time\nwhile True:\n    time.sleep(3600)\n' > "$smoke_payload"
mise exec -- go run ./cmd/yeet --host yeet-lab run dns-smoke-a "$smoke_payload" --net=svc
mise exec -- go run ./cmd/yeet --host yeet-lab run dns-smoke-b "$smoke_payload" --net=svc
mise exec -- go run ./cmd/yeet --host yeet-lab ssh dns-smoke-a -- getent hosts dns-smoke-b
mise exec -- go run ./cmd/yeet --host yeet-lab ssh dns-smoke-a -- getent hosts dns-smoke-b.yeet.internal
```

Expected: both lookups return the private svc IP for `dns-smoke-b`.

- [ ] **Step 6: Smoke VM DNS on yeet-lab**

Create or reuse two `svc` VMs:

```bash
mise exec -- go run ./cmd/yeet --host yeet-lab run dns-vm-a vm://nixos/unstable --net=svc
mise exec -- go run ./cmd/yeet --host yeet-lab run dns-vm-b vm://nixos/unstable --net=svc
mise exec -- go run ./cmd/yeet --host yeet-lab ssh dns-vm-a -- getent hosts dns-vm-b
mise exec -- go run ./cmd/yeet --host yeet-lab ssh dns-vm-a -- getent hosts dns-vm-b.yeet.internal
```

Expected: both lookups return the private svc IP for `dns-vm-b`.

- [ ] **Step 7: Smoke svc,lan routing on yeet-lab**

Run:

```bash
mise exec -- go run ./cmd/yeet --host yeet-lab run dns-vm-lan vm://nixos/unstable --net=svc,lan
mise exec -- go run ./cmd/yeet --host yeet-lab ssh dns-vm-lan -- resolvectl domain
mise exec -- go run ./cmd/yeet --host yeet-lab ssh dns-vm-lan -- getent hosts dns-vm-a
mise exec -- go run ./cmd/yeet --host yeet-lab ssh dns-vm-lan -- getent hosts example.com
```
...
```bash
mise exec -- go run ./cmd/yeet --host yeet-lab rm dns-smoke-a --yes
mise exec -- go run ./cmd/yeet --host yeet-lab rm dns-smoke-b --yes
mise exec -- go run ./cmd/yeet --host yeet-lab rm dns-vm-a --yes
mise exec -- go run ./cmd/yeet --host yeet-lab rm dns-vm-b --yes
mise exec -- go run ./cmd/yeet --host yeet-lab rm dns-vm-lan --yes
rm -f "${smoke_payload:-}"
```
