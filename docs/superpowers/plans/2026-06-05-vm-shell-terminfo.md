...
**Tech Stack:** Go metadata injection in `pkg/catch`, catch bootstrap prereq checks in `cmd/catch`, Bash startup files, ncurses `tic`/terminfo, Go unit tests, live pve1 VM test.
...
- [ ] **Step 2: Redeploy catch and live-test pve1**

Run the updated catch on `yeet-pve1`, create a fresh disposable VM, and verify:
...
Run: `yeet rm <test-vm>@yeet-pve1 --yes --clean-data --clean-config`
