...
**Files:**
- Runtime-only changes on `root@cloud-host`
- Runtime-only changes on `root@lab-host`
...
```bash
ssh root@cloud-host "grep -E '^(PartOf|Requires|After|ExecStart)=' /etc/systemd/system/yeet-dns.service"
ssh root@lab-host "grep -E '^(PartOf|Requires|After|ExecStart)=' /etc/systemd/system/yeet-dns.service"
```
...
```bash
ssh root@cloud-host "before=\$(systemctl show -p MainPID --value yeet-dns.service); systemctl restart catch.service; sleep 2; after=\$(systemctl show -p MainPID --value yeet-dns.service); printf 'cloud-host before=%s after=%s\n' \"\$before\" \"\$after\"; test \"\$before\" != \"\$after\""
ssh root@lab-host "before=\$(systemctl show -p MainPID --value yeet-dns.service); systemctl restart catch.service; sleep 2; after=\$(systemctl show -p MainPID --value yeet-dns.service); printf 'lab-host before=%s after=%s\n' \"\$before\" \"\$after\"; test \"\$before\" != \"\$after\""
```
...
```bash
ssh root@cloud-host "systemctl is-active catch yeet-dns.service && ss -lunp 'sport = :53' && dig @192.168.100.1 beszel A +short && dig @192.168.100.1 uptime-kuma.yeet.internal A +short"
ssh root@lab-host "systemctl is-active catch yeet-dns.service && ss -lunp 'sport = :53' && dig @192.168.100.1 nixbox A +short && dig @192.168.100.1 nixbox.yeet.internal A +short"
```
