...
rg -n 'root@lab-host|yeet-lab|/Users/' README.md website/docs
```
...
- Uses: committed `yeet init`, `yeet vm images prune`, VM status/info/SSH/console/snapshot commands, systemd, `/proc`, and the approved `root@lab-host` host.
...
From the committed isolated tree, record service names, unit hashes, systemd `MainPID`, Firecracker PID/UID/GID, start time, readiness marker, image bundle, service root, disk backend, and reachability for every VM. Use `CATCH_HOST=yeet-lab` for Yeet commands and `ssh root@lab-host` only for host evidence. Save this inventory outside the public repository.
...
```bash
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet vm images prune --dry-run
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet vm images prune --yes
```
