...
```bash
CATCH_HOST=yeet-pve1 go run ./cmd/yeet init root@yeet-pve1
```

Expected: install completes without hanging and reports catch installed.

- [ ] **Step 5: Recreate a pve1 LAN VM and immediately SSH**

Run:

```bash
CATCH_HOST=yeet-pve1 go run ./cmd/yeet rm devbox --clean-data
CATCH_HOST=yeet-pve1 go run ./cmd/yeet run devbox@yeet-pve1 vm://ubuntu/26.04 --service-root=flash/yeet/vms/devbox --zfs --disk=128g --net=lan
CATCH_HOST=yeet-pve1 go run ./cmd/yeet ssh devbox -- true
```
