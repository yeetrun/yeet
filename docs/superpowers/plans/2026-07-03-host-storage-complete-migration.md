...
func TestApplyHostStorageConfigMovesClearsDefaultServiceRoot(t *testing.T) {
	...
	updated, skipped := applyHostStorageConfigMoves(cfg, "yeet-lab", "/flash/yeet/services", []catchrpc.HostStorageServiceMove{{
		Name: "nginx",
...
```sh
yeet --host yeet-lab host set --zfs \
...
- [ ] **Step 5: Live repair validation on `yeet-lab`**

Deploy catch from the workspace:

```sh
mise exec -- go run ./cmd/yeet --host yeet-lab --progress=plain init root@lab-host
```

Run the repair-aware host set command:

```sh
mise exec -- go run ./cmd/yeet --host yeet-lab host set --zfs \
  --data-dir=flash/yeet/data \
  --services-root=flash/yeet/services \
  --migrate-services=all \
...
```sh
ssh root@lab-host 'bash -lc "grep -R /root/data /etc/systemd/system /run/systemd/system 2>/dev/null || true"'
```
...
```sh
ssh root@lab-host 'bash -lc "docker ps -q | xargs -r docker inspect --format \"{{.Name}} {{range .Mounts}}{{.Source}} {{end}}\" | grep /root/data || true"'
```
...
```sh
mise exec -- go run ./cmd/yeet --host yeet-lab status --progress=plain
mise exec -- go run ./cmd/yeet info --host=yeet-lab
```
