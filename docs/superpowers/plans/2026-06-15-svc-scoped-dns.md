...
### Task 7: Live Smoke Test On pve1 And hetz
...
print("pve1 docker config ok")
PY'
ssh root@hetz 'python3 - <<'"'"'PY'"'"'
import json
cfg=json.load(open("/etc/docker/daemon.json"))
assert cfg.get("features", {}).get("containerd-snapshotter") is True, cfg
assert "dns" not in cfg, cfg
assert "dns-search" not in cfg, cfg
print("hetz docker config ok")
PY'
```
...
services:
  client:
    ...
    command: ["sh", "-c", "sleep 1d"]
YAML
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet run codex-dns-target "$tmpdir/codex-dns-target.yml" --net=svc
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet run codex-dns-client "$tmpdir/codex-dns-client.yml" --net=svc
```
...
```bash
ssh root@pve1 'cid=$(docker ps -q --filter label=com.docker.compose.project=catch-codex-dns-client --filter label=com.docker.compose.service=client); docker exec "$cid" getent hosts codex-dns-target; docker exec "$cid" getent hosts codex-dns-target.yeet.internal; docker exec "$cid" getent hosts example.com'
```
...
services:
  client:
    ...
    command: ["sh", "-c", "sleep 1d"]
YAML
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet run codex-dns-lan "$tmpdir/codex-dns-lan.yml" --net=lan
ssh root@pve1 'cid=$(docker ps -q --filter label=com.docker.compose.project=catch-codex-dns-lan --filter label=com.docker.compose.service=client); docker exec "$cid" cat /etc/resolv.conf; docker exec "$cid" getent hosts example.com'
```
...
```bash
ssh root@pve1 'for project in catch-prowlarr catch-radarr catch-sonarr catch-sabnzbd; do cid=$(docker ps -q --filter label=com.docker.compose.project=$project | head -n1); printf "%s " "$project"; docker exec "$cid" getent hosts google.com >/dev/null && echo ok; done'
```
...
```bash
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet remove codex-dns-target --yes --clean-data --clean-config
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet remove codex-dns-client --yes --clean-data --clean-config
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet remove codex-dns-lan --yes --clean-data --clean-config
rm -rf "$tmpdir"
```
