...
**Goal:** Build and publish a faster Ubuntu 26.04 VM image path with Rust `yeet-init`, kernel-level first-interface networking, early systemd-managed SSH readiness, and correct guest reboot behavior, then prove the improvement on `pve1`.
...
**Tech Stack:** Go, Rust, Cargo, mise, Bash, systemd, Firecracker, Linux direct kernel boot, GitHub Actions, ZFS live validation on `pve1`.
...
- `yeet run "$svc@yeet-pve1" vm://ubuntu/26.04 --service-root="flash/yeet/vms/$svc" --zfs --disk=128g --net=lan` returns only after `yeet-ready <iface> <ip>` has appeared after the current unit start.
- An immediate `yeet ssh "$svc@yeet-pve1" -- true` succeeds after `yeet run` returns.
...
## Task 7: Full Verification, GitHub Build, and Live pve1 Measurements
...
- [ ] **Step 5: Build local yeet and install catch on pve1**

```bash
go build -o /tmp/yeet-vmfast ./cmd/yeet
/tmp/yeet-vmfast init root@pve1
```

Expected: catch installs successfully on `pve1`.
...
log=".tmp/${svc}-run.log"
start="$(date +%s.%N)"
YEET_TRACE=1 /tmp/yeet-vmfast run "$svc@yeet-pve1" vm://ubuntu/26.04 \
...
```bash
/tmp/yeet-vmfast ssh "$svc@yeet-pve1" -- true
/tmp/yeet-vmfast ssh "$svc@yeet-pve1" -- systemd-analyze | tee ".tmp/${svc}-systemd-analyze.txt"
/tmp/yeet-vmfast ssh "$svc@yeet-pve1" -- systemd-analyze blame | head -n 30 | tee ".tmp/${svc}-systemd-blame.txt"
/tmp/yeet-vmfast ssh "$svc@yeet-pve1" -- dmesg | tail -n 80 | tee ".tmp/${svc}-dmesg-tail.txt"
/tmp/yeet-vmfast ssh "$svc@yeet-pve1" -- systemctl status yeet-sshd.service yeet-guest-ready.service --no-pager | tee ".tmp/${svc}-guest-services.txt"
```
...
```bash
/tmp/yeet-vmfast ssh "$svc@yeet-pve1" -- sudo reboot || true
sleep 3
ssh root@pve1 "systemctl is-active yeet-vm-$svc.service"
timeout 45 sh -c 'until /tmp/yeet-vmfast ssh "'"$svc@yeet-pve1"'" -- true >/dev/null 2>&1; do sleep 1; done'
```
...
printf 'y\ny\n' | /tmp/yeet-vmfast rm "$svc@yeet-pve1" --clean-data
remove_end="$(date +%s.%N)"
python3 - <<PY
...
PY
ssh root@pve1 "zfs list -H -o name | grep -F 'flash/yeet/vms/$svc' || true"
```
...
- Spec coverage: The plan covers Rust via mise, a Rust `yeet-init`, kernel first-interface networking, systemd-managed SSH readiness, the guest reboot fix, image workflow building through GitHub, pve1 live testing, clean removal measurement, no snap support documentation, and yeet-managed kernel documentation.
