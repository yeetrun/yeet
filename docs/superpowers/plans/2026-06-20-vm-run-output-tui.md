...
func TestRunUIPrintBlockPlainEmitsInfoLines(t *testing.T) {
	var buf bytes.Buffer
	ui := newRunUI(&buf, false, false, "run", "devbox@yeet-lab")
	...
	want := []string{
		`action=run service=devbox@yeet-lab status=info detail=devbox@yeet-lab`,
...
func TestRunUIPrintBlockQuietSuppressesOutput(t *testing.T) {
	var buf bytes.Buffer
	ui := newRunUI(&buf, false, true, "run", "devbox@yeet-lab")

	ui.PrintBlock([]string{"devbox@yeet-lab"})
...
func TestRunUIDockerPlainOutputRemainsStructured(t *testing.T) {
	var buf bytes.Buffer
	ui := newRunUI(&buf, false, false, "run", "api@yeet-lab")
	...
	want := []string{
		...
		`action=run service=api@yeet-lab status=running step="Detect payload"`,
		`action=run service=api@yeet-lab status=ok step="Detect payload" detail="docker compose"`,
		`action=run service=api@yeet-lab status=running step="Install service"`,
		`action=run service=api@yeet-lab status=ok step="Install service"`,
	}
	if !reflect.DeepEqual(lines, want) {
...
- [ ] **Step 4: Install local catch on `yeet-lab` for smoke testing**
...
Expected: install succeeds and reports the catch service installed on `root@lab-host`.
...
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet run "$svc" vm://ubuntu/26.04 --net=svc,lan --image-policy=cached --cpus=1 --memory=1g --disk=8g
```
...
CATCH_HOST=yeet-lab mise exec -- go run ./cmd/yeet ssh "$svc" -- sh -lc 'hostname; ip route get 1.1.1.1'
```
...
ssh root@lab-host "test ! -e /root/data/services/$svc && echo remote-service-root-removed"
```
