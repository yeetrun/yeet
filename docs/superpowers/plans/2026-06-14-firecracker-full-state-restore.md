...
### Task 5: Local Verification And Live pve1 Smoke Tests
...
chmod +x "$payload"
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet run "$svc" "$payload" --service-root="$dataset" --zfs --snapshots=on --progress=plain
ssh root@pve1 "sh -lc 'set -e; mkdir -p \"$root/data\"; printf snapshot > \"$root/data/smoke.txt\"'"
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet snapshots create "$svc" --comment="codex service-root restore smoke"
snap="$(CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet snapshots list "$svc" --format=json | jq -r --arg svc "$svc" '[.[] | select(.service == $svc and .comment == "codex service-root restore smoke")] | sort_by(.created) | last | .shortName')"
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet snapshots inspect "$svc" "$snap"
ssh root@pve1 "sh -lc 'set -e; printf mutated > \"$root/data/smoke.txt\"; printf stale > \"$root/data/stale-after-snapshot.txt\"'"
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet snapshots restore "$svc" "$snap" --stop --yes --progress=plain
ssh root@pve1 "sh -lc 'set -e; test \"$(cat \"$root/data/smoke.txt\")\" = snapshot; test ! -e \"$root/data/stale-after-snapshot.txt\"; test \"$(zfs get -H -o value mountpoint \"$dataset\")\" = \"$root\"'"
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet remove "$svc" --yes --clean-data --clean-config
ssh root@pve1 "sh -lc 'zfs destroy -r \"$dataset\" >/dev/null 2>&1 || true; rm -rf \"$root\" >/dev/null 2>&1 || true'"
...
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet run "$vm" vm://nixos/26.05 --net=svc --image-policy=cached --cpus=1 --memory=1g --disk=16g --snapshots=on --progress=plain
for i in $(seq 1 60); do
	if CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet ssh "$vm" -- true >/dev/null 2>&1; then break; fi
	sleep 2
done
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet ssh "$vm" -- sh -lc 'printf snapshot > /root/disk-smoke; sync'
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet snapshots create "$vm" --comment="codex vm disk restore smoke"
snap="$(CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet snapshots list "$vm" --format=json | jq -r --arg vm "$vm" '[.[] | select(.service == $vm and .comment == "codex vm disk restore smoke")] | sort_by(.created) | last | .shortName')"
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet snapshots list "$vm"
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet snapshots inspect "$vm" "$snap"
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet ssh "$vm" -- sh -lc 'printf mutated > /root/disk-smoke; printf stale > /root/stale-after-snapshot; sync'
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet snapshots restore "$vm" "$snap" --mode=disk --stop --start --yes --progress=plain
for i in $(seq 1 60); do
	if CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet ssh "$vm" -- true >/dev/null 2>&1; then break; fi
	sleep 2
done
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet ssh "$vm" -- sh -lc 'test "$(cat /root/disk-smoke)" = snapshot; test ! -e /root/stale-after-snapshot'
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet remove "$vm" --yes --clean-data --clean-config
rm -f yeet.toml
```
...
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet ssh "$vm" -- sh -lc 'printf disk-before > /root/full-disk-smoke; printf ram-before > /run/full-ram-smoke; nohup sh -c "while true; do date +%s > /run/full-ram-heartbeat; sleep 1; done" >/dev/null 2>&1 & sync'
sleep 3
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet snapshots create "$vm" --full --comment="codex vm full restore smoke"
snap="$(CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet snapshots list "$vm" --format=json | jq -r --arg vm "$vm" '[.[] | select(.service == $vm and .comment == "codex vm full restore smoke" and .mode == "full")] | sort_by(.created) | last | .shortName')"
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet snapshots inspect "$vm" "$snap"
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet ssh "$vm" -- sh -lc 'printf disk-after > /root/full-disk-smoke; printf ram-after > /run/full-ram-smoke; rm -f /run/full-ram-heartbeat; sync'
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet snapshots restore "$vm" "$snap" --mode=full --stop --yes --progress=plain
for i in $(seq 1 60); do
	if CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet ssh "$vm" -- true >/dev/null 2>&1; then break; fi
	sleep 2
done
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet ssh "$vm" -- sh -lc 'test "$(cat /root/full-disk-smoke)" = disk-before; test "$(cat /run/full-ram-smoke)" = ram-before; test -s /run/full-ram-heartbeat; pgrep -f full-ram-heartbeat >/dev/null'
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet remove "$vm" --yes --clean-data --clean-config
rm -f yeet.toml
```
...
```bash
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet snapshots list "$vm" > /tmp/yeet-list.txt
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet snapshots inspect "$vm" "$snap" > /tmp/yeet-inspect.txt
...
```bash
rm -f yeet.toml
ssh root@pve1 "sh -lc 'systemctl list-units \"yeet-vm-codex-*\" --no-legend --all || true; zfs list -H -o name | grep \"^flash/yeet/codex-\" || true'"
but status -fv
```
...
- [ ] Live smoke verifies disk state and RAM-only state on `yeet-pve1`.
