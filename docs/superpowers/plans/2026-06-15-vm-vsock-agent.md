...
## Task 8: Deploy Catch and Run pve1 Live Smoke Test

**Files:**
- No source edits unless live testing reveals a defect.

- [ ] **Step 1: Install updated catch correctly on pve1**
...
Expected: catch installs successfully and keeps the real service flags, including `--data-dir=/root/data --tsnet-host=yeet-pve1`.
...
```bash
ssh root@pve1 'systemctl is-active catch && systemctl cat catch | sed -n "/^ExecStart=/p"'
```

Expected: output includes `active` and an `ExecStart=` line with `--data-dir=/root/data --tsnet-host=yeet-pve1`.
...
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet run "$svc" vm://ubuntu/26.04 --net=lan --image-policy=cached --cpus=1 --memory=1g --disk=8g
```
...
```bash
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet ip "$svc"
```
...
```bash
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet info "$svc" --format=json-pretty | jq '.network.ips, .vm.networks'
```
...
```bash
CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet --no-tty ssh "$svc" -- hostname
```
...
```bash
ip="$(CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet ip "$svc" | head -n1)"
ssh root@pve1 "ip -4 neigh del '$ip' dev vmbr0 2>/dev/null || true"
```
...
Expected: VM service is removed. If remove flags have changed, run `CATCH_HOST=yeet-pve1 mise exec -- go run ./cmd/yeet remove --help-llm` and use the documented equivalent.
...
```text
- catch version deployed to pve1 through yeet init
...
- Spec coverage: the plan covers Firecracker vsock config, separate `yeet-agent`, host-initiated JSONL protocol, port `7788`, IPv4-only v1, live source priority for `ip` and `ssh`, legacy fallbacks, no stale DHCP authority, image integration, Rust/Go tests, and pve1 smoke verification.
